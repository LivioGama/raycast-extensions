...
export interface EngineOptions {
  prompt: string;
  model: string;
  provider?: "openai" | "perplexity" | "ollama" | "raycast";
  webSearch?: boolean;
  apiKeys?: {
    openai?: string;
    perplexity?: string;
...
export async function convertPrompt(
  ...
  const isRaycast = provider === "raycast";
  ...
  const apiKey = isOpenAI
    ...
    ? apiKeys.perplexity
    : isRaycast
    ? undefined // Raycast AI doesn't require an API key
    : apiKeys.ollama;
  
  if (!apiKey && !isOllama && !isRaycast) {
    return {
      success: false,
  ...
  try {
    ...
    const resolvedBaseURL = isRaycast
      ? "https://backend.raycast.com/api/v1/ai"
      : isOllama
    ...
    const client = new OpenAI({
      apiKey: isOllama 
        ? (apiKey?.trim() || "ollama") 
        : isRaycast 
        ? "raycast-ai" // Raycast AI doesn't require a real API key
